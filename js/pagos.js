document.addEventListener('DOMContentLoaded',()=>{
  ...
  document.getElementById('qrBtn').onclick=()=>{
    paymentSection.innerHTML=''; const p=document.createElement('p'); p.textContent='Escanea el código para pagar:';
    const img=document.createElement('img'); const rest=order.total-(order.amountPaid||0); img.src=`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent('Pagar $'+rest.toFixed(2))}`; img.alt='QR'; img.style.display='block'; img.style.margin='10px auto';
    const confirm=document.createElement('button'); confirm.textContent='Confirmar pago'; confirm.onclick=()=>finalize('qr');
    paymentSection.appendChild(p); paymentSection.appendChild(img); paymentSection.appendChild(confirm);
  };
