document.addEventListener('DOMContentLoaded',()=>{
  ...
  function renderMenu(){ products=loadProducts(); menuS.innerHTML='';
    ...
    const thead=document.createElement('thead'); const trh=document.createElement('tr'); ['Categoría','Nombre','Precio','Existencias','Acciones'].forEach(h=>{ const th=document.createElement('th'); th.textContent=h; trh.appendChild(th); }); thead.appendChild(trh); table.appendChild(thead);
    ...
    products.forEach((p,idx)=>{ const r=document.createElement('tr');
      ...
      const std=document.createElement('td'); const sin=document.createElement('input'); sin.type='number'; sin.min='0'; sin.step='1'; sin.value=p.stock||0; std.appendChild(sin); r.appendChild(std);
    ...
    const nstock=document.createElement('td'); const sinp=document.createElement('input'); sinp.type='number'; sinp.min='0'; sinp.step='1'; sinp.placeholder='Stock'; nstock.appendChild(sinp); nr.appendChild(nstock);
    const nact=document.createElement('td'); const add=document.createElement('button'); add.type='button'; add.textContent='Agregar'; add.onclick=()=>{ const cat=nsel.value, nm=ninp.value.trim(), pr=parseFloat(pinp.value||'0'), st=parseInt(sinp.value||'0'); if(!nm||isNaN(pr)) { alert('Completa nombre y precio'); return; } const id=`${cat.toLowerCase()}-${Date.now()}`; products.push({id,category:cat,name:nm,price:pr,stock:st,image:'assets/images/placeholder.png'}); saveProducts(products); renderMenu(); }; nact.appendChild(add); nr.appendChild(nact);
    tbody.appendChild(nr); table.appendChild(tbody);
    const save=document.createElement('button'); save.type='button'; save.textContent='Guardar cambios'; save.onclick=()=>{ const rows=[...tbody.children]; const updated=[]; rows.slice(0,rows.length-1).forEach((row,idx)=>{ const cat=row.children[0].firstChild.value; const nm=row.children[1].firstChild.value.trim(); const pr=parseFloat(row.children[2].firstChild.value); const st=parseInt(row.children[3].firstChild.value||'0'); updated.push({...products[idx],category:cat,name:nm,price:pr,stock:st}); }); products=updated; saveProducts(products); alert('Productos actualizados.'); renderMenu(); };
